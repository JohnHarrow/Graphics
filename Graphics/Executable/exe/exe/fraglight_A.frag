...
vec4 global_ambient = vec4(0.05, 0.05, 0.05, 1.05);
vec4 fambientcolour;
int  shininess = 8;
...
void main()
{
	...
	outputColor = attenuation*(fambientcolour + diffuse + specular) * texcolour + emissive + global_ambient;
	//outputColor =  attenuation * (texcolour + specular) + emissive + global_ambient;
